...
from train.lora import train as train_lora
...
@app.command(help="Fine Tune Stable Diffusion with LoRA")
def lora(
    input_images_dir: Annotated[
        str, typer.Option(help="Path to folder containing training data")
    ],
    instance_prompt: Annotated[
        str,typer.Option(help="The prompt with identifier specifying the instance, e.g. 'a photo of a jkcgprnna wearing casual clothes, smiling'" 
        ),
    ],
    base_model: Annotated[
        str, typer.Option(help="Base Model to train StableDiffusion was trained on",),
    ] = "stabilityai/stable-diffusion-xl-base-1.0",
    pretrained_vae: Annotated[
        str, typer.Option(help="VAE model with better numerical stability",),
    ] = "madebyollin/sdxl-vae-fp16-fix",
    resolution: Annotated[
        int,typer.Option(help="The resolution for input images, all the images will be resized to this",
        ),
    ] = 1024,
    ...
    max_train_steps: Annotated[
        int,typer.Option(help="Total number of training steps to run for, more your images, more should be this value",
        ),
    ] = 500,
    gradient_accumulation_steps: Annotated[
        int,typer.Option(help="Number of update steps to accumulate before performing a backward pass",
        ),
    ] = 1,
    learning_rate: Annotated[
        float,typer.Option(help="Initial learning rate for training, after warmup period"),
    ] = 1e-4,
    use_8bit_adam: Annotated[
        bool,typer.Option(help="Whether or not to use 8-bit Adam from bitsandbytes. Ignored if optimizer is not set to AdamW",
        ),
    ] = False,
    use_tf32: Annotated[
        bool,typer.Option(help="Whether or not to allow TF32 on Ampere GPUs. Can be used to speed up training.",
        ),
    ] = False,
    mixed_precision: Annotated[
        MixedPrecisionType,typer.Option(
...
def infer(
    prompt: Annotated[
        str,typer.Option(
            help=" The prompt for text to image, e.g. 'a photo of a jkcgprnna wearing casual clothes, smiling' "
        ),
    ],
    ...
    base_model: Annotated[
        str, typer.Option(help="Base Model your StableDiffusion was trained on",),
    ] = "stabilityai/stable-diffusion-xl-base-1.0",
    pretrained_vae: Annotated[
        str, typer.Option(help="VAE model with better numerical stability",),
    ] = "madebyollin/sdxl-vae-fp16-fix",
